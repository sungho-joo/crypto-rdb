...
class TickerDBRepository:
    ...
    def select_from_tables(self, stmt: sa.sql.Select) -> Tuple[Dict[int, Tuple[Any, ...]], List[str]]:
    ...
    def get_all_market_codes(self) -> List[str]:
        ...
        stmt = sa.select(Ticker.market_code)
    ...
    def get_some_market_codes(self, ids: List[str]) -> List[str]:
        ...
        stmt = sa.select(Ticker.market_code, Ticker.market_code.in_(ids))
    ...
    def get_some_market_ids(self, codes: List[int]) -> List[int]:
        ...
        stmt = sa.select(Ticker.id).where(Ticker.market_code.in_(codes))
        return [ticker_id[0] for ticker_id in self.select_from_tables(stmt)[0].values()]
    ...
    def get_price_ranges(
        ...
        stmt = sa.select(Trade.trade_date, Trade.trade_time, Trade.trade_price).where(
            sa.and_(
    ...
    def get_acc_ask_volume(
        ...
        stmt = (
            sa.select(Trade.trade_date, Trade.trade_time, Accum.acc_ask_volume)
            .join(Accum, Trade.id == Accum.id)
            .where(
                sa.and_(
                    Trade.ticker_id == ticker_id,
    ...
    def get_acc_bid_volume(
        ...
        stmt = (
            sa.select(Trade.trade_date, Trade.trade_time, Accum.acc_bid_volume)
            .join(Accum, Trade.id == Accum.id)
            .where(
                sa.and_(
                    Trade.ticker_id == ticker_id,
